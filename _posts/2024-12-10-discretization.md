...
The Euler method can be viewed as approximating ODE trajectories piecewise by straight segments, where each step uses the tangent line at the current point to approximate the local curve. This local straight-line approximation is a natural choice for rectified flow induced by straight interpolation. However, if the rectified flow is induced by a curved interpolation scheme, it may be more natural to use local curve segments—derived from the same interpolation scheme—instead of straight lines. We refer to such approximation schemes as **natural Euler samplers**.

This blog will first introduce the idea of **natural Euler samplers** and then show that the trajectories of natural Euler samplers are equivariant for pointwise transformable interpolations:

> If two interpolation processes are pointwise transformable into one another, then the discrete trajectories points of their corresponding natural Euler samplers are also pointwise transformable using the same mapping functions, provided their time grids are related by the time scaling function $\tau_t$.

This result implies that, when natural Euler samplers are employed, **changing the affine interpolation scheme is essentially equivalent to altering the time grid**. Importantly, we emphasize that this equivalence is **not** restricted to the continuous-time ODE trajectories. Even after discretization via Euler steps, once the sampled points are transformed accordingly, the resulting discrete points will match numerically.

## Natural Euler Samplers

Consider an interpolation process $$X_t=\texttt I_t(X_0, X_1)$$ with a rectified flow velocity field $v_t$. In the vanilla Euler method, the trajectories of the flow are approximated on a discrete time gird $\{t_i\}_i$ as:

$$
\hat z_{t_{i+1}} = \hat z_{t_i} + (t_{i+1} - t_i) v_t(\hat z_{t_i}),
$$

Note: Define the interpolation
...
>Example: Curverd Euler Sampler for Spherical Interpolation
>
>$$
...
> Example: Curved Euler Sampler for DDIM
>
> The discretized inference scheme of DDIM is an instance of natural Euler sampler. Note that the inference update of DDIM is written in terms of the expected noise $$\hat{x}_0\vert_t(x) = \mathbb{E}[X_0\vert X_t = x]$$. Hence,
>
> $$
> \begin{aligned}
...
>
> If the time grids satisfy $$\tau(t_i') = t_i$$ for all $$i$$, and the initial conditions align via $$\hat{z}_{t_0}' = \phi(\hat{z}_{t_0})$$, then the **discrete** trajectories match under the transform:
>
> $$
...
$$
\texttt{NaturalEuler}(\texttt{Rectify}(T(\{X_t\}))) = T(\texttt{NaturalEuler}(\texttt{Rectify}(\{X_t\}))).
$$
...
>Example: Equivalence of Straight Euler and DDIM sampler
>
