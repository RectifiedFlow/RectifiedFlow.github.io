...
Given a coupling $$(X_0, X_1)$$ of the noise $$X_0 \sim \pi_0$$ and data $$X_1 \sim \pi_1$$, rectified flow first leverages an interpolation process $$\{X_t : t \in [0,1]\}$$ to smoothly connect $$X_0$$ and $$X_1$$. It then "causalizes" or "rectifies" this interpolation into an ODE-based generative model:

$$
\mathrm{d}Z_t = v_t(Z_t)  \mathrm{d}t, \quad Z_0 = X_0, \quad  \text{with velocity field} \quad v_t(x) = \mathbb{E}[\dot{X}_t \mid X_t = x],
$$

where $$\dot{X}_t$$ denotes the time derivative of the process $$X_t$$. This construction of $$v_t$$ ensures that the marginal distribution of $$Z_t$$ matches that of $$X_t$$, while enables causal generation of $$Z_1$$ forward in time $t$ starting from the noise $$Z_0 = X_0$$.
In practice, $$v_t$$ is estimated by minimizing a mean square loss $$\int_0^1 \mathbb{E}[\left \| \dot X_t - v_t(X_t)\right \|^2] \mathrm{d} t$$. 
