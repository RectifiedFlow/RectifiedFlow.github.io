...
description: Rectified flow learns ODEs as generative models while favoring straightness, by causalizing (or rectifying) an interpolation process that smoothly connects noise and data.
...
$$
\hat{Z}_{t+\epsilon} = \hat{Z}_t + \epsilon \cdot v_t(\hat{Z}_t), \quad \forall t \in \{0, \epsilon, 2\epsilon, \dots, 1\}, \tag{2}
$$
