...
- Hence in Fig.2(b), at these intersection points of $$\{X_t\}$$ where the direction is otherwise uncertain, the ODE “derandomizes” the update direction by following the conditional expectation $$\displaystyle v_t^*(X_t) = \mathbb{E}[\dot{X}_t \mid X_t].$$
