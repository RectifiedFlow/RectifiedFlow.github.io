...
The first step is to build an interpolation process $$\{X_t\} = \{X_t : t \in [0, 1]\}$$ that smoothly interpolates between $$X_0$$ and $$X_1$$. Given the preference on straight trajectories, there seems no immediate reason to not use the canonical straight-line interpolation:

$$
X_t = t X_1 + (1 - t) X_0.X_t = t X_1 + (1 - t) X_0.
$$
...
- In contrast, by the definition of an ODE $$\dot{Z}_t = v_t^*(Z_t)$$, the update direction $$\dot{Z}_t$$ at each point $$Z_t$$ is uniquely determined by $$Z_t$$. Hence, It is impossible for two trajectories of the ODE to intersect each other.
