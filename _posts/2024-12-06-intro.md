...
thumbnail: /assets/img/thumbnail/intro_thumbnail.png
...
One option is to favor ODEs that are easy to solve at inference time. In practice, the ODEs are approximated using numerical methods, which typically construct piecewise linear approximations of the ODE trajectories. For instance, a common choice is Euler method:
...
The first step is to build an interpolation process $$\{X_t\} = \{X_t : t \in [0, 1]\}$$ that smoothly interpolates between $$X_0$$ and $$X_1$$. Given the perference on straight trajectories, there seems no immediate reason to not use the canonical straight-line interpolation:
...
which is the conditional expectation of the slope $$\dot{X}_t$$ for all the interpolation trajectories passing through a given point $$X_t = x$$.
...
- Different trajectories of the interpolation process $$\{X_t\}$$ can intersect as shown in Fig.2(a), causing multiple possible values of $$\dot{X}_t$$ for the same point $$X_t$$. 
  
- In contrast, by the definition of an ODE $$\dot{Z}_t = v_t^*(Z_t)$$, the update direction $$\dot{Z}_t$$ at each point $$Z_t$$ is uniquely determined by $$Z_t$$. Hence, it is impossible to have two trajectories of the ODE to across each other. 
  
- Hence at these intersection points of $$\{X_t\}$$ where the direction is otherwise uncertain, the ODE $$\{Z_t\}$$ “derandomizes” the update direction by following the conditional expectation $$\displaystyle v_t^*(X_t) = \mathbb{E}[\dot{X}_t \mid X_t].$$ Consequently, the trajectories of the ODE "reassemble" the interpolation trajectories in a way that avoids intersections. See Fig.2(b). 
...
Intuitively, by the definition of $$v_t$$ in (2), the total amount of mass flow entering and exiting every infinitesimal volume in the space is equal under the dynamics of $$X_t$$ and $$Z_t$$. This ensures that the two processes yield the same marginal distributions, even though the flow directions may differ. 
...
As shown in [Fig.2 (c)](#figure-2), after applying the "Reflow" operation, the trajectories become starighter than the original rectified flow $$Z_t$$.

> **Reflow and Shoftcut Learning.** Intuitively, reflow resembles shortcut learning in humans: once we solve a problem for the first time, we learn to go directly to the solution, enabling us to solve it more quickly the next time. 
{: .example}
