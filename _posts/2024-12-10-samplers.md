...
Given a coupling $$(X_0,X_1)$$ of noise and data points, rectified flow takes an interpolation process, such as $$X_t = t X_1 + (1-t) X_0$$, and "rectifies" or "caualizes" it to yield an ODE model $$\mathrm d Z_t = v_t(Z_t) \mathrm d t$$ initialized from $$Z_0 = X_0$$, with the velocity is $$v_t(z) = \mathbb{E}[\dot X_t | X_t = z]$$, and estimated by minimizing loss $$\mathbb{E}_{t,X_0,X_1}\left[\left\|\dot{X}_t-v_t(X_t) \right \| ^2 \right]$$. 
...
Applying Langevin dynamics with a stochastic sampler reduces errors compared to using a deterministic sampler, such as Euler. As shown below, the stochastic sampler achieves improved results due to this correction.
...
This formulation recovers both the SDE of DDPM and the score-based SDEs when $$\gamma_t = 1 / \alpha_t$$, yielding 

$$
\mathrm{d} Z_t = 2 v_t(Z_t) \, \mathrm{d} t - \frac{\dot{\alpha}_t}{\alpha_t} Z_t \, \mathrm{d} t + \sqrt{2 \, \frac{\lambda_t \beta_t \gamma_t}{\alpha_t}} \, \mathrm{d} W_t.
$$
