...
<div class="l-page-outset">
  <figure id="figure-3">
    <iframe src="{{ '/assets/img/flux_text_rendering.png' | relative_url }}" 
            frameborder="0" 
            scrolling="no" 
            height="50px" 
            width="100%">
    ...
    <figcaption>
      ...
      Left: results of stochastic sampler to the Flux model consistently outperforms the deterministic Euler sampler in text rendering performance across all step sizes. Right: examples showcasing the enhanced text rendering quality achieved with the stochastic sampler.
    </figcaption>
  </figure>
</div>
...
## Noises Cause Over-Concentration
...
This is indeed the case in practice. As shown in Figure.4 below, when we increase the noise magnitude $$\sigma_t$$,  the generated samples to cluster closer to the centers of the Gaussian modes.

<div class="l-page-outset">
  <figure id="figure-3">
    <iframe src="{{ '/assets/img/sde_concentrate_2d.png' | relative_url }}" 
            frameborder="0" 
            scrolling="no" 
            height="50px" 
            width="100%">
    ...
    <figcaption>
      <a href="#figure-3">Figure 3</a>.
      Using larger diffusion coefficient $$\sigma_t$$ yields more concentrated samples. 
    </figcaption>
  </figure>
</div>


So, large diffusion yields more concentrate results. This is rather counter-intuitive. Why is it the case? To see this, let us exam how to estimate the score function from the estimated velocity field $$\hat v_t$$: 
$$
\nabla \log \hat \rho_t(x) = \frac{1}{\lambda_t \beta_t} \left( \alpha_t \hat v_t(x) - \dot{\alpha}_t x \right). 
$$
In the case of straight interpolation, we have $$\nabla \log \hat \rho_t(x) = \frac{t \hat v_t(x) - x}{1-t}$$.
Note that  $$\beta_t $$ must converge to 0 when $$t\to 1$$. Hence, $$\nabla \log \hat \rho_t(x)$$ will most likely diverge to infinity as $$t\to 1$$. This would almost certainly yield a large overestimation of the true magnitude of $$\nabla \log \rho_t(x)$$, because the true $$\nabla \log \rho_t(x)$$ can be finite. As a result, the results from the SDE tends to be more concentrated than what it should be, because $$\nabla \log \rho_t$$ tends to point towards the center of the mass (local optima of the probability density). 


On text-to-image generation, this over-concentration effect tends to yield overly smoothing images, often yield Cartoon-ish images.  This over-smoothing effect results in outputs that lack fine details and high-frequency variations, leading to a blurred appearance. The figure below illustrates these differences. The samples generated by Euler sampler has more high-frequency details (See the parrot feather and smoke). 
