...
description: How to use stochastic samplers from flow? Why are the pros and cons?
tags: tutorial
...
To see this, let us exam how to estimate the score function from the estimated velocity field $$\hat v_t$$: 
...
Because $$\beta_t$$ must converge to 0 as $$t \to 1$$, the estimated score function $$\nabla \log \hat{\rho}_t(x)$$ is likely to diverge to infinity in this limit. Consequently, this leads to a significant overestimation of the true magnitude of $$\nabla \log \rho_t(x)$$, as the true $$\nabla \log \rho_t(x)$$ can remain finite. As a result, the outputs of the SDE tend to become more concentrated than they should be. This is because $$\nabla \log \rho_t$$ increasingly points toward the center of the mass (i.e., the local optima of the probability density).
